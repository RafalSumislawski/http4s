...
private final class Http1Connection[F[_]](
  ...
  private def executeRequest(req: Request[F], idleTimeoutF: F[TimeoutException], idleRead: Option[Future[ByteBuffer]]): F[Response[F]] = {
        ...
        F.defer {
          ...
          idleTimeoutF.start.flatMap { timeoutFiber =>
            ...
            val response: F[Response[F]] = writeRequest.start >>
              receiveResponse(mustClose, doesntHaveBody = req.method == Method.HEAD, idleTimeoutS, idleRead)
