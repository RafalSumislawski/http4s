...
private final class Http1Connection[F[_]](
  ...
  def runRequest(req: Request[F], idleTimeoutF: F[TimeoutException]): F[Response[F]] =
    F.defer[Response[F]] {
      stageState.get match {
        case New =>
          if (stageState.compareAndSet(New, ReadWrite)) {
            logger.debug(s"Connection was new. Running.")
            executeRequest(req, idleTimeoutF, None)
          } else {
            logger.debug(s"Connection changed state since checking it was new. Looping.")
        ...
        case i @ Idle(idleRead) =>
          if (stageState.compareAndSet(i, ReadWrite)) {
            ...
            executeRequest(req, idleTimeoutF, Some(idleRead))
          } else {
